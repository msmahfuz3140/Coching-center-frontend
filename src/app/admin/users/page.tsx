      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="relative rounded-3xl overflow-hidden shadow-xl" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 50%, #4f46e5 100%)' }}>
          ...
          <div className="relative px-8 py-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <p className="text-indigo-300 text-sm font-medium tracking-wide">Admin Panel</p>
              <h1 className="mt-1 text-2xl sm:text-3xl font-extrabold text-white">Manage Users</h1>
              <p className="mt-1 text-indigo-200 text-sm">View, manage roles, block or grant course access</p>
            </div>
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div key={s.label} className={`px-4 py-2 rounded-2xl ${s.color} text-center min-w-[72px]`}>
          ...
          <div className="flex gap-2">
            {(['ALL', 'STUDENT', 'ADMIN'] as const).map(r => (
              <button key={r} onClick={() => setFilterRole(r)}
                className={`px-4 py-2.5 rounded-xl text-sm font-semibold transition-all ${filterRole === r ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  ...
                  <tr key={user.id} className={`hover:bg-gray-50/60 transition-colors ${isUpdating === user.id ? 'opacity-50' : ''}`}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-sm">
                          {user.name?.charAt(0)?.toUpperCase() || 'U'}
                        </div>
                        <span className="font-semibold text-gray-900 text-sm">{user.name || 'Unnamed'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 max-w-[200px] truncate">{user.email}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${user.role === 'ADMIN' ? 'bg-violet-100 text-violet-700' : 'bg-blue-100 text-blue-700'}`}>
                        {user.role === 'ADMIN' ? '👑' : '🎓'} {user.role}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${user.emailVerified ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                          {user.emailVerified ? '✓ Verified' : '⏳ Pending'}
                        </span>
                        {user.isBlocked && <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">🚫 Blocked</span>}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl border border-gray-200 hover:border-violet-300 hover:bg-violet-50 text-gray-500 hover:text-violet-600 transition-all disabled:opacity-40"
                        >
                        ...
                        <button
                          ...
                          title="Course Access"
                          className="p-2 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 text-gray-500 hover:text-blue-600 transition-all"
                        >
